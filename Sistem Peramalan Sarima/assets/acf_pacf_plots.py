...
def acf_pacf_plot(_arr, _upper_ci, _lower_ci, is_acf=True):
    ...
    # Add CI
    fig.add_scatter(x=np.arange(len(_arr[0])), y=_upper_ci, mode='lines', line_color='rgba(255,255,255,0)', hoverinfo='none')
    fig.add_scatter(x=np.arange(len(_arr[0])), y=_lower_ci, fill='tonexty', fillcolor='rgba(178, 211, 194,0.11)', line_color='rgba(255,255,255,0)', hoverinfo='none')
...
def acf_pacf_resid_plot(_arr, _upper_ci, _lower_ci, is_acf=True):
    fig = go.Figure(layout=my_figlayout)
    _hovertemplate = (
        "<i>Lag</i>: %{x}<br>"+
        "<i>Correlation</i>: %{y:.3f}"+
    ...
    fig.update_yaxes(range=[-1.1,1.1])
    if is_acf:
        fig.update_layout(title="Residual Autocorrelation (ACF)")
    else:
        fig.update_layout(title="Residual Partial Autocorrelation (PACF)")
    fig.update_yaxes(zerolinecolor='#000000')

    return fig

def acf_pacf(_df, _colname, num_lags):
    acf_corr_array = acf(list(_df[_colname]), nlags=num_lags, alpha=0.05)  # Includes 95% CI
    acf_lower_ci = acf_corr_array[1][:, 0] - acf_corr_array[0]
    acf_upper_ci = acf_corr_array[1][:, 1] - acf_corr_array[0]
    pacf_corr_array = pacf(list(_df[_colname]), nlags=num_lags, alpha=0.05)  # Includes 95% CI
    pacf_lower_ci = pacf_corr_array[1][:, 0] - pacf_corr_array[0]
    pacf_upper_ci = pacf_corr_array[1][:, 1] - pacf_corr_array[0]
...
def acf_pacf_resid(_df, _colname, num_lags):
    acf_corr_array = acf(list(_df[_colname]), nlags=num_lags, alpha=0.05)  # Includes 95% CI
    acf_lower_ci = acf_corr_array[1][:, 0] - acf_corr_array[0]
    acf_upper_ci = acf_corr_array[1][:, 1] - acf_corr_array[0]
    pacf_corr_array = pacf(list(_df[_colname]), nlags=num_lags, alpha=0.05)  # Includes 95% CI
    ...
    # Plot ACF
    fig_acf = acf_pacf_resid_plot(acf_corr_array, acf_upper_ci, acf_lower_ci, True)
    fig_pacf = acf_pacf_resid_plot(pacf_corr_array, pacf_upper_ci, pacf_lower_ci, False)
